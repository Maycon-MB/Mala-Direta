...
def generate_pdf(file_path, data):
    ...
    font_size_name_cep = 12  # Tamanho da fonte para nome e CEP
    font_size_default = 10  # Tamanho de fonte padrão
    ...
    for i, item in enumerate(data):
        ...
        if i % 3 == 0:
            pdf.set_font("Helvetica", size=font_size_name_cep)
        else:
            pdf.set_font("Helvetica", size=font_size_default)

        pdf.multi_cell(item_width, 15, item, border=0, align="L")
...
def etiqueta_Nascimento():
    ...
    for eleitor in eleitores_filtrados:
        ...
        etiqueta_text = (
            f"{eleitor[1].upper()}\n"
            f"{eleitor[2]}\n"
            f"{eleitor[4]}, {eleitor[5]}, {eleitor[6]}\n"
            f"{eleitor[7]}, {eleitor[8]} - {eleitor[9]}\n"
            f"{cep_formatado}\n"
        )

        etiqueta_data.append(etiqueta_text)

    # Defina o tamanho de fonte antes de chamar generate_pdf
    font_size_name_cep = 12
    font_size_default = 10
    font_sizes = [font_size_name_cep if i == 0 or i == 4 else font_size_default for i in range(len(etiqueta_data))]
    
    generate_pdf(pdf_filename, etiqueta_data, font_sizes)
