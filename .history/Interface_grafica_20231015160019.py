...
def etiqueta_semNascimento():
    global pessoas_registradas  # Use a variável global de eleitores em vez de eleitores filtrados

    pessoas_registradas.sort(key=lambda eleitor: eleitor[1].lower())

    pdf_filename = "etiquetas-sem-nascimento.pdf"

    # Criação do texto da etiqueta sem nascimento
    etiqueta_data = []
    for eleitor in pessoas_registradas:  # Use todos os eleitores, não apenas os filtrados
        cep_formatado = formatar_cep(eleitor[3])
