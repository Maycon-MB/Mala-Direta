...
def criar_janela_exibir_eleitores(total_eleitores):
    ...
    layout_fecharExibicao = [
        [sg.Table(values=(dados_formatados), headings=['Matrícula', 'Nome', 'Nascimento', 'CEP','Logradouro', 'Número', 'Complemento', 'Bairro', 'Cidade', 'UF', 'E-mail', 'Telefone', 'Instagram', 'OBS']background_color='lightgrey', auto_size_columns=False,
              justification='center', key='-OUTPUT-', num_rows=min(50, len(dados_formatados)), expand_x=True, expand_y=True, size =(800,400)), ],
...
while True:
    ...
    if eventos == 'Exibir Eleitores':
        ...
        def alternar_cores_tabela():
                    for i in range(len(data)):
                        # Determine a cor de fundo com base no índice da linha
                        cor_fundo = 'lightgray' if i % 2 == 0 else 'white'
                        # Atualize a cor de fundo da célula na coluna 'Matrícula'
                        janela_eleitores['-OUTPUT-'].update(data[i][0], background_color=cor_fundo)

                # Chame a função para alternar as cores da tabela
        alternar_cores_tabela()
