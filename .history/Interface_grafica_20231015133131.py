...
def generate_pdf(file_path, data):
    ...
    eleitor4_index = data.index("eleitor[4]") if "eleitor[4]" in data else None
    eleitor9_index = data.index("eleitor[9]") if "eleitor[9]" in data else None
    ...
    for i, item in enumerate(data):
        ...
        elif i == eleitor9_index:
            pdf.set_font("Arial", size=11)  # Define a fonte como Arial com tamanho 11
        else:
            pdf.set_font("Arial", size=9)  # Define a fonte como Arial com tamanho 9

        pdf.set_xy(x, y)
        pdf.multi_cell(item_width, item_height / 3, item, border=0, align="L")  # Alinha o texto à esquerda

        if i == eleitor4_index:
            pdf.ln(2)  # Reduz o espaço vertical para 2 entre as linhas
        else:
            pdf.ln(5)  # Espaçamento padrão de 5 entre as linhas
