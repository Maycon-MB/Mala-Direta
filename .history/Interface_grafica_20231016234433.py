...
def generate_pdf(filename, data):
    c = PDF(filename, pagesize=letter)
    width, height = letter

    # Tamanho do retângulo para cada etiqueta
    rect_width = width / 3
    rect_height = height / 9.3  # Ajuste o valor conforme necessário
    ...
    for label in data:
        ...
        lines = label.split('\t\t')  # Divide o texto em várias linhas usando '\t\t'

        for line in lines:
            c.drawString(x + 5, y - rect_height + 5, line)
            c.set_line_height(rect_height)  # Define o espaçamento entre as linhas
