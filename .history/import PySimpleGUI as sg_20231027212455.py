...
while True:
    ...
    if event == '-TABLE-':
        if values['-TABLE-'] and values['-TABLE-'][0] is not None:
            # Obtém a linha clicada na tabela
            row, col = values['-TABLE-']

            # Verifica se o evento foi um clique duplo (double click)
            if 'DOUBLECLICK' in event:
                # Abra uma caixa de diálogo de entrada para a edição
                novo_valor = sg.popup_get_text('Editar valor:', default_text=dados_eleitores[row])
                if novo_valor is not None:
                    # Atualize o valor na lista de dados
                    dados_eleitores[row] = novo_valor
