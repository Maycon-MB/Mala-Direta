...
def relatorio_bairro():
    ...
    # Ordene os eleitores em ordem alfabética pelo nome (índice 1)
    eleitores_filtrados.sort(key=lambda eleitor: eleitor[7].lower())
...
def criar_janela_exibir_eleitores(total_eleitores):
    ...
    layout_botoesSegundaTela = [
        ...
        sg.Button('Limpar Filtros', expand_x=True, expand_y=True, key='-LIMPAR-FILTROS-', size=(10, 1)), sg.Button('Deletar Eleitor', expand_x=True, expand_y=True, size=(10, 1)),  sg.Button('Fechar', expand_x=True, expand_y=True, size=(10, 1)), ],
    ]
