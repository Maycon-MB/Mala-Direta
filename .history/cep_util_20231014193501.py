...
def formatar_cep(valor_atual):
    # Certifique-se de que valor_atual seja uma string
    valor_atual = str(valor_atual)
    
    # Remova caracteres não numéricos e mantenha apenas dígitos
    valor_formatado = ''.join(filter(str.isdigit, valor_atual))

    # Verifique se o valor formatado tem pelo menos 8 dígitos e, em seguida, formate-o como um CEP
