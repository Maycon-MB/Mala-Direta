...
def generate_pdf(file_path, data):
    ...
    # Defina a fonte padrão e tamanho da fonte
    ...
    item_height = page_height / 9.3

    # Ajuste o espaçamento vertical (para baixo) e o espaçamento horizontal (para a direita)
    vertical_spacing = 0 # Ajuste o valor conforme necessário
    horizontal_spacing = 0  # Não adicione espaçamento horizontal
    ...
    for i, item in enumerate(data):
        ...
        col = i % 3
        row = (i % 27) // 3
        x = col * item_width + horizontal_spacing
        y = row * item_height + vertical_spacing

        lines = item.split('\n')
        reduce_spacing = False  # Variável para controlar a redução do espaçamento

        for line in lines:
            if "eleitor[1]" in line.lower() or "cep_formatado" in line.lower():
                pdf.set_font("Arial", size=11)
            else:
            ...
            # Remova a borda definindo 'border' para 0
            pdf.multi_cell(item_width, item_height / 6, line, border=0, align="L")

            if "eleitor[4]" in line:
                reduce_spacing = True

            if reduce_spacing:
                y += item_height / 3 - 3  # Reduz o espaço vertical entre as linhas
            else:
                y += item_height / 3 - 7 # Espaçamento padrão entre as linhas
