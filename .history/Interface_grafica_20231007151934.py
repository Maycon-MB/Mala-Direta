...
# Defina as cores de fundo para linhas pares e ímpares
background_color_odd = 'lightblue'  # Cor de fundo das linhas ímpares
background_color_even = 'white'  # Cor de fundo das linhas pares

# Crie uma lista de elementos de tabela para cada linha de dados com cores alternadas
table_rows = []
for i, row_data in enumerate(dados_formatados):
    if i % 2 == 0:
        # Linha par
        row = [sg.TableText(text, background_color=background_color_even, justification='center') for text in row_data]
    else:
        # Linha ímpar
        row = [sg.TableText(text, background_color=background_color_odd, justification='center') for text in row_data]
...
def criar_janela_exibir_eleitores(total_eleitores):
    ...
    layout_fecharExibicao = [
        [sg.Table(values=dados_formatados, headings=['Matrícula', 'Nome', 'Nascimento', 'CEP', 'Logradouro', 'Número', 'Complemento', 'Bairro', 'Cidade', 'UF', 'E-mail', 'Telefone', 'Instagram', 'OBS'],
                    auto_size_columns=False, justification='center', key='-OUTPUT-', num_rows=min(50, len(dados_formatados)),
                    expand_x=True, expand_y=True, size=(800, 400),
                    background_color=(background_color_odd, background_color_even),
                    text_color='white', alternating_row_color='lightgray')],
