...
def generate_pdf(file_path, data):
    ...
    # Define as dimensões das colunas para ocupar a página inteira
    ...
    for i, item in enumerate(data):
        ...
        # Defina a posição e ajuste a largura da célula
        pdf.set_xy(x, 0)

        # Defina diferentes tamanhos de fonte para o texto
        if i == 1 or i == 2:
            pdf.set_font("Helvetica", size=12)
        else:
            pdf.set_font("Helvetica", size=10)

        pdf.multi_cell(column_width, 8, item, border=0, align="C")  # Ajuste a altura da célula conforme necessário

    # Salve o PDF no caminho especificado
    pdf.output(file_path)

def etiqueta_Nascimento(eleitores_filtrados):
    ...
    print(f"Etiqueta com nascimento criada com sucesso: {pdf_filename}")
