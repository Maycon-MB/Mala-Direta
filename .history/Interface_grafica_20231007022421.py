...
from funcoes_compartilhadas import deletar_porID, filtra filtrar_por_bairro, filtrar_por_cep, filtrar_por_cidade, filtrar_por_complemento, filtrar_por_email, filtrar_por_instagram, filtrar_por_logradouro, filtrar_por_nascimento, filtrar_por_nome, filtrar_por_numero, filtrar_por_telefone, filtrar_por_uf
...
def generate_pdf(file_path, data):
    ...
    item_width = page_width / 3  # 3 columns
    item_height = page_height / 9  # 9 rows
    ...
    for i, item in enumerate(data):
        # Calculate the position (x, y) for the current item
        x = (i % 3) * item_width
        y = (i // 3) * item_height

        # Set the position for the item and add it to the PDF
        pdf.set_xy(x, y)
...
layout_coluna1 = [
    ...
    [sg.Text('Data de nascimento', font=font),  sg.Text('(dd/mm)', size=(16,1), font=("Helvetica", 10))], [sg.InputText(key='nascimento', size=(30, 1), font=font, enable_events=True)],
    [Text('CEP', size=(10, 1), font=font)], [sg.InputText(key='cep', size=(30, 1), font=font, enable_events=True), Button('Consultar CEP')]
]

layout_coluna2 = [
    [Text('Logradouro', size=(10, 1),   font=font)], [sg.InputText(key='logradouro', size=(30, 1), font=font)],
    [Text('Número', size=(10, 1), font=font)], [sg.InputText(key='numero', size=(30, 1), font=font)],
    [Text('Complemento', size=(11, 1), font=font)], [sg.InputText(key='complemento', size=(30, 1), font=font)],
]

layout_coluna3 = [
    [Text('Bairro', size=(10, 1), font=font)], [sg.InputText(key='bairro', size=(30, 1), font=font)],
