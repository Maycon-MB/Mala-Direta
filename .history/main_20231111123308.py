...
from database import exibir_eleitores, registrar_eleitor, atualizar_registro, executar_query
...
def criar_janela_exibir_eleitores(total_eleitores):
    ...
    layout_fecharExibicao = [
        [sg.Table(values=(dados_formatados), headings=['Matrícula', 'Nome', 'Nascimento', 'CEP','Logradouro', 'Número', 'Complemento', 'Bairro', 'Cidade', 'UF', 'E-mail', 'Telefone', 'Instagram', 'OBS', 'Zona', 'Seção', 'Local'], auto_size_columns=True,
            justification='center', vertical_scroll_only = False, key='-OUTPUT-',text_color='black', background_color='#C7CEC9', alternating_row_color='#3DA5FF', num_rows=min(25, len(dados_formatados)), expand_x=True, expand_y=True, size=(550, 400), select_mode='browse', col_widths=25), 
        ],
...
def criar_janela_edicao(data, id):
    layout = [
...
def atualizar_tabela(janela, data):
    # Execute uma consulta SQL que ordena os registros por nome
    query = "SELECT * FROM tabela_eleitores ORDER BY nome"
    conexao, cursor = database.executar_query(query)  # Chame a função no módulo database
    data = cursor.fetchall()
    janela['-OUTPUT-'].update(values=data)
    conexao.close()
...
# Defina a função para filtrar os eleitores6
...
layout_login = [
    ...
    [sg.Input(key="-SENHA-", password_char="*", size=25), sg.Button("Mostrar / Ocultar"),],
    [sg.Button("Entrar")]
]
...
while True:
    ...
    if event2 == '\r':
    # Quando Enter é pressionado, mova o foco para o próximo elemento
        current_focus4 = (current_focus4 + 1) % len(focusable_elements4)
        elemento_com_foco = focusable_elements4[current_focus4]
        janela_login[focusable_elements4        
        [current_focus4]].set_focus()
        ...
        while True:
            ...
            if eventos_eleitores == sg.WINDOW_CLOSED or eventos_eleitores == 'Fechar':
                ...
                if last_highlighted_row is not None:
                    janela_eleitores['-OUTPUT-'].update(select_rows=[last_highlighted_row], background_color='white')
                last_highlighted_row = None
                # Chame a função criar_janela_edicao diretamente
                edit_window = criar_janela_edicao(eleitores_filtrados[selected_index], selected_id)

              
                if eventos_eleitores == 'Editar Tabela':
                    if valores_eleitores['-OUTPUT-'] and len(valores_eleitores['-OUTPUT-']) > 0:
                        selected_index = int(valores_eleitores['-OUTPUT-'][0])  # Converta para inteiro
                        if 0 <= selected_index < len(eleitores_filtrados):
                            selected_id = eleitores_filtrados[selected_index][0]  # Obtenha o ID do registro selecionado
                            edit_window = criar_janela_edicao(eleitores_filtrados[selected_index], selected_id)
                        else:
                            sg.popup_error('Índice selecionado está fora dos limites.')
                    else:
                        sg.popup_error('Nenhum índice selecionado.')
            ...
            if eventos_eleitores == '-LIMPAR-FILTROS-':
                ...
                atualizar_contagem_eleitores()  # Chame a função para atualizar a contagem de eleitores
            ...
            if eventos_eleitores == 'Filtrar por Local':
                ...
                if local_filtro:
                    ...
                    janela_eleitores['-OUTPUT-'].update(values=eleitores_filtrados)
                    atualizar_contagem_eleitores()  # Atualizar a contagem de eleitores
                    selected_index = None  # Reinicie o índice selecionad
