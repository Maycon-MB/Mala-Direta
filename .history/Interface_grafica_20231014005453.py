...
def generate_rectangles_pdf(file_path):
    pdf = PDF()
    pdf.add_page()
...
def generate_pdf(file_path, data):
    generate_rectangles_pdf(file_path)
    ...
    for i, item in enumerate(data):
        ...
        # Reduzir o tamanho da fonte para caber no retângulo
        pdf.set_font("Helvetica", size=10)

        x = (i % 3) * rect_width
        y = (int(i / 3)) * rect_height
        pdf.set_xy(x, y)

        # Ajustar o tamanho da célula para caber no retângulo
        pdf.multi_cell(rect_width, 10, item, border=0, align="L")
