...
def generate_pdf(file_path, data):
    pdf = PDF()
    pdf.add_page()
    pdf.set_font("Helvetica", size=10)  # Define o tamanho padrão da fonte como 10
    ...
    # Defina a largura das colunas para ocupar toda a largura da página dividida por 3
    item_width = page_width / 3

    # Desative as margens para eliminar qualquer espaço entre as colunas
    ...
    for i, item in enumerate(data):
        ...
        for line in lines:
            if "eleitor[1]" in line or "cep_formatado" in line:  # Verifica os campos que precisam de tamanho diferente
                pdf.set_font("Arial", size=12)  # Define a fonte como Arial com tamanho 12
            else:
                pdf.set_font("Helvetica", size=10)  # Define a fonte como Helvetica com tamanho 10
