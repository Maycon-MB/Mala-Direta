...
from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, PageBreak, Spacer, FRAME
...
def generate_pdf(file_path, data):
    ...
    # Define os estilos para o texto
    ...
    for i, item in enumerate(data):
        ...
        row = (i % 27) // 3
        x = col * rect_width
        y = height - (row * rect_height + rect_height)

        # Desenha o retângulo
        frame = Frame(x, y, rect_width, rect_height, showBoundary=1)
        story.append(frame)
        ...
        for line in lines:
            ...
            p.wrapOn(doc, rect_width, rect_height)
            p.drawOn(doc, x, y + rect_height - p.height)
