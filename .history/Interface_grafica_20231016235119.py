...
class PDF(canvas.Canvas):
    def set_line_height(self, height):
        self._leading = height

def generate_pdf(filename, data):
    c = canvas.Canvas(filename, pagesize=letter)
    width, height = letter

    # Define o tamanho da fonte e a fonte a ser usada
    font_size = 12
    font_name = "Helvetica"

    # Carrega a fonte e define o tamanho da fonte
    pdfmetrics.registerFont(TTFont(font_name, "Helvetica.ttf"))
    c.setFont(font_name, font_size)

    # Define o espaçamento entre linhas com base no tamanho da fonte
    leading = pdfmetrics.getLeading(font_name, font_size)

    rect_width = width / 3
    rect_height = height / 9.3
    ...
    for label in data:
        ...
        for i, line in enumerate(lines):
            # Calcula a posição da linha com base no espaçamento entre linhas
            line_y = y - rect_height + 5 - (i * leading)
...
def etiqueta_Nascimento():
    ...
    for eleitor in eleitores_filtrados:
        ...
        etiqueta_text = (
            f"{eleitor[1].upper()}\t\t{eleitor[4]}, {eleitor[5]}, {eleitor[6]}\t{eleitor[7]}, {eleitor[8]} - {eleitor[9]}\t\t{cep_formatado}                               {eleitor[2]}"
        )
...
def etiqueta_Geral():
    ...
    for eleitor in eleitores_completos:
        ...
        etiqueta_text = (
            f"{eleitor[1].upper()}\t\t{eleitor[4]}, {eleitor[5]}, {eleitor[6]}\t{eleitor[7]}, {eleitor[8]} - {eleitor[9]}\t\t{cep_formatado}"
        )
