...
def draw_rectangles(c, num_columns, num_items_per_column):
    page_width, page_height = letter

    column_width = page_width / num_columns
    item_height = page_height / num_items_per_column

    for x in range(0, int(page_width), int(column_width)):
        for y in range(0, int(page_height), int(item_height)):
            c.rect(x, y, column_width, item_height)

def generate_pdf(file_path, data):
    c = canvas.Canvas(file_path, pagesize=letter)

    num_columns = 3
    num_items_per_column = 27

    # Defina as dimensões da página
    page_width, page_height = letter

    # Desenhe os retângulos
    draw_rectangles(c, num_columns, num_items_per_column)

    # Preencha os dados nos retângulos
    current_x, current_y = 0, page_height
    item_height = page_height / num_items_per_column

    for i, item in enumerate(data):
        if i > 0 and i % num_items_per_column == 0:
            c.showPage()
            current_x = 0
            current_y = page_height - item_height

        if "eleitor[1]" in item.lower() or "cep_formatado" in item.lower():
            c.setFont("Helvetica", 11)
        else:
            c.setFont("Helvetica", 9)

        c.drawString(current_x + 5, current_y - 5, item)
