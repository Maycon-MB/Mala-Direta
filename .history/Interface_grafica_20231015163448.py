...
for line in lines:
    pdf.set_font("Arial", size=11) if any(e in line for e in ["eleitor[4]", "eleitor[5]", "eleitor[6]", "eleitor[7]", "eleitor[8]", "eleitor[9]"]) else pdf.set_font("Arial", size=9)

    pdf.set_xy(x, y)
    pdf.multi_cell(item_width, item_height / 3, line, border=0, align="L")  # Alinha o texto à esquerda

    if any(e in line for e in ["eleitor[4]", "eleitor[5]", "eleitor[6]", "eleitor[7]", "eleitor[8]", "eleitor[9]"]):
