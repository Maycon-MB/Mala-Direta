...
def generate_pdf(file_path, data):
    ...
    for i, item in enumerate(data):
        ...
        x = ((i % 27) % 3) * item_width
        y = (((i % 27) // 3) + 0.01) * item_height

        # Define tamanhos de fonte diferentes para campos específicos
        pdf.set_xy(x, y)
        pdf.set_font("Helvetica", size=12)  # Tamanho de fonte 12
        pdf.multi_cell(item_width, 8, item.split("\n")[0], border=0, align="L")

        pdf.set_font("Helvetica", size=11)  # Volta ao tamanho de fonte 11
        pdf.multi_cell(item_width, 8, "\n".join(item.split("\n")[1:]), border=0, align="L")
