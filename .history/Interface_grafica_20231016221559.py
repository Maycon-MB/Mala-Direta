...
def generate_pdf(file_path, data, with_nascimento=False):
    ...
    for i, item in enumerate(data):
        ...
        for line in lines:
            ...
            c.drawString(current_x, current_y - item_height, line)

            current_y -= item_height

            if current_y < 0:
                current_x += column_width + horizontal_spacing
                current_y = page_height
...
def etiqueta_Nascimento():
    ...
    generate_pdf(pdf_filename, etiqueta_data, with_nascimento=True)
...
def etiqueta_Geral():
    ...
    generate_pdf(pdf_filename, etiqueta_data, with_nascimento=False)
