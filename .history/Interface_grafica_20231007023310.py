...
class MyPDF(FPDF):
    def header(self):
        # Define o cabeçalho da página (opcional)
        self.set_font("Helvetica", "B", 12)
        self.cell(0, 10, "Meu Relatório", 0, 1, "C")
...
def generate_pdf(file_path, data):
    ...
    for i, item in enumerate(data):
     ...
     # Valor para mover todo o conteúdo para baixo (ajuste conforme necessário)
        vertical_offset = 5  # Você pode ajustar esse valor

        # Calcule a posição (x, y) para o item atual na página atual
        x = ((i % 27) % 3) * item_width
        y = ((((i % 27) // 3) + 1) * item_height) + vertical_offset
