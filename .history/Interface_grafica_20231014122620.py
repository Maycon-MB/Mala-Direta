...
def generate_pdf(file_path, data):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Helvetica", size=10)  # Tamanho de fonte padrão
    ...
    for i, item in enumerate(data):
        ...
        x = ((i % 27) % 3) * item_width
        y = (((i % 27) // 3) + 0.1) * item_height  # Ajuste a coordenada Y para maior espaço

        # Defina a posição e adicione o item à página atual
        pdf.set_xy(x, y)

        # Verifique o índice do item para definir o tamanho da fonte
        if i % 27 == 0 or (i - 3) % 27 == 0:  # Índices 0, 3, 27, 30, 54, etc.
            pdf.set_font("Helvetica", size=12)  # Tamanho de fonte maior
        else:
            pdf.set_font("Helvetica", size=10)  # Tamanho de fonte padrão

        pdf.multi_cell(item_width, 8, item, border=0, align="L")  # Ajuste o tamanho do item
