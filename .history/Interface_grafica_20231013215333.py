...
def generate_pdf(file_path, data):
    ...
    for i, item in enumerate(data):
        ...
        # Calcule a coluna atual
        coluna_atual = i % 3

        # Define o tamanho da fonte para campos específicos
        if coluna_atual == 0:  # Primeira coluna
            pdf.set_font("Helvetica", size=12)
        else:
            pdf.set_font("Helvetica", size=11)

        # Alinhe o texto à esquerda
        x = coluna_atual * item_width
        y = (i % 27 // 3) * item_height
        pdf.set_xy(x, y)
        pdf.multi_cell(item_width, 10, item, border=0, align="L")
