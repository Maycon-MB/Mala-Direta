...
def generate_pdf(file_path, data):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Helvetica", size=10)  # Fonte 10 como padrão
    ...
    rect_width = page_width / 3  # 3 colunas
    rect_height = page_height / 9  # 9 linhas

    # Define a fonte 12 para campos específicos
    ...
    for i, item in enumerate(data):
        ...
        # Calcule a posição (x, y) para o retângulo atual na página atual
        x = ((i % 27) % 3) * rect_width
        y = ((i % 27) // 3) * rect_height
        ...
        x_text = x + 5
        y_text = y + 5  # Ajuste para evitar que o texto fique muito próximo às bordas do retângulo

        # Define a posição e adicione o item à página atual
        pdf.set_xy(x_text, y_text)
        pdf.multi_cell(rect_width - 10, 12, item, border=0, align="C")  # Fonte 12 para o texto
