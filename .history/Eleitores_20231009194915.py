...
def registrar_eleitor(valores):
    comando = """
        INSERT INTO tabela_eleitores (
            nome, nascimento, cep, logradouro, numero, complemento, bairro, cidade, uf, email, telefone, instagram, obs
        ) VALUES (
    ...
    cadastro_eleitor = (
        ...
        valores['uf'], valores['email'], valores['telefone'], valores['instagram'], valores['obs']
    )
...
def filtrar_por_email(email):
    comando = "SELECT * FROM tabela_eleitores WHERE email ILIKE %s"
    conexao, cursor = executar_query(comando, (f"%{email}%",))
    resultados = cursor.fetchall()
    cursor.close()
    conexao.close()
    return resultados
...
def filtrar_por_telefone(telefone):
    comando = "SELECT * FROM tabela_eleitores WHERE telefone ILIKE %s"
    conexao, cursor = executar_query(comando, (f"%{telefone}%",))
    resultados = cursor.fetchall()
    cursor.close()
    conexao.close()
    return resultados
...
def filtrar_por_instagram(instagram):
    comando = "SELECT * FROM tabela_eleitores WHERE instagram ILIKE %s"
    conexao, cursor = executar_query(comando, (f"%{instagram}%",))
    resultados = cursor.fetchall()
    cursor.close()
    conexao.close()
    return resultados